import pygame, sys, random, hashlib, ranking, user_password
from pygame.math import Vector2

def update_ranking(mode, player_name, score):
    player_lis = ranking.player_dic[f'{mode}_player_lis']

    if player_name in player_lis:
        if ranking.score_dic[f'{mode}_score_lis'][player_lis.index(player_name)] < score:
            ranking.score_dic[f'{mode}_score_lis'][player_lis.index(player_name)] = score
    else:
        player_lis.append(player_name)
        ranking.score_dic[f'{mode}_score_lis'].append(score)

    ranking.player_dic[f'{mode}_player_lis'] = player_lis
    game_window.mode = mode
    game_window.sort_ranking()

    with open('ranking.py', mode='w', encoding='utf-8') as f:
        f.write('player_dic = '+str(ranking.player_dic)+'\nscore_dic = '+str(ranking.score_dic))
...
def login(change=False):
    ...
    if username == '' or password == '':
        ...
        if username not in user_password.user_password:
            ...
            blit_help_time = 120
                                 ...
                                 hashlib.pbkdf2_hmac('sha512', str.encode(password), 
                                                     str.encode(password), 5), 20) != user_password.user_password[username]:
...
def sign_up():
    ...
    elif username in user_password.user_password or user_password in ['admin', 'Admin', 'ADMIN', 'root', 'Root', 'ROOT', 'Guest', 'guest', 'GUEST']:
            ...
            if mode == 'down':
                if (event.type == pygame.MOUSEBUTTONDOWN) or check:
                    if (pygame.mouse.get_pos()[0] > Rect_x and pygame.mouse.get_pos()[0] < Rect_x+230
                        ...
                        login(change) # change: 确定
                        if blit_help_time != 120:
                            if change:
                                user_password.user_password[real_username] = hashlib.pbkdf2_hmac('sha512', str.encode(password), 
                                                                                                 hashlib.pbkdf2_hmac('sha512', str.encode(password), 
                        ...
                        sign_up()
                        if blit_help_time != 120:
                            user_password.user_password[username] = hashlib.pbkdf2_hmac('sha512', str.encode(password), 
                                                                                        hashlib.pbkdf2_hmac('sha512', str.encode(password), 
                                                                                                            str.encode(password), 5), 20)
                            with open('user_password.py', mode='w', encoding='utf-8') as f:
                                f.write('user_password = '+str(user_password.user_password))
                            shut_down = True
                            break
                    check = False
...
class SNAKE:
    def __init__(self):
        self.reset()
        self.crunch_sound = pygame.mixer.Sound('sound/贪知蛇吃食物.wav')
        
        self.head_image = pygame.transform.smoothscale(pygame.image.load('image/头.png').convert_alpha(), (size, size))
        self.body_image = pygame.transform.smoothscale(pygame.image.load('image/身体.png').convert_alpha(), (size, size))
        self.tail_image = pygame.transform.smoothscale(pygame.image.load('image/尾巴.png').convert_alpha(), (size, size))
        self.convert_image = pygame.transform.smoothscale(pygame.image.load('image/转接处.png').convert_alpha(), (size, size))
...
class MAIN:
    def __init__(self, mode='class', fast=False, reset_fruit=False, teach=True, only_add_sub=False
        ...
        self.over_music = pygame.mixer.Sound('sound/贪知蛇死亡.wav')
        self.play_bgm()
    ...
    def create_wiki_topic(self):
        global answer_lis
        self.wiki_num = random.randint(0, len(wiki_topic_lis))
        answer_lis = wiki_answer_lis[self.wiki_num*3-3:self.wiki_num*3]
        self.topic = wiki_topic_lis[self.wiki_num-1]
        self.two_lines = False
...
class GAME_WINDOW:
                        ...
                        if self.choose_index == 0: # math
                            ...
                            while True:
                                while True:
                                    ...
                                    index = 0
                                    for name in ranking.player_dic[f'{mode}_player_lis']:
                                        if name == username:
                                    ...
                                    index = 0
                                    for score in ranking.score_dic[f'{mode}_score_lis']:
                                        if ranking.player_dic[f'{mode}_player_lis'][index] == username:
                                    ...
                                    if self.choose_index == 0: # personal ranking
                                        ...
                                        while True:
                                            ...
                                            for mode in mode_text_lis:
                                                if username in ranking.player_dic[f'{mode_lis[mode_text_lis.index(mode)]}_player_lis']:
                                                    count += 1
                                            ...
                                            for mode in mode_text_lis:
                                                if username in ranking.player_dic[f'{mode_lis[mode_text_lis.index(mode)]}_player_lis']:
                                                    ...
                                                    rank = ranking.player_dic[f'{mode_lis[mode_text_lis.index(mode)]}_player_lis'].index(username) + 1
                                                    rank_text = ' 第' + str(rank) + '名'
                                                    score = str(ranking.score_dic[f'{mode_lis[mode_text_lis.index(mode)]}_score_lis'][ranking.player_dic[f'{mode_lis[mode_text_lis.index(mode)]}_player_lis'].index(username)]) + rank_text
    ...
    def sort_ranking(self):
        if ranking.player_dic[f'{self.mode}_player_lis']:
            dic = dict(zip(ranking.score_dic[f'{self.mode}_score_lis'], ranking.player_dic[f'{self.mode}_player_lis']))
            ranking.score_dic[f'{self.mode}_score_lis'].sort(reverse=True)

            for i in range(len(ranking.score_dic[f'{self.mode}_score_lis'])):
                ranking.player_dic[f'{self.mode}_player_lis'][i] = dic[ranking.score_dic[f'{self.mode}_score_lis'][i]]
...
apple = pygame.image.load('image/apple20_20.png').convert_alpha()
...
bgm = pygame.mixer.Sound('sound/贪知蛇背景音乐.mp3')

font_size = int(round(pix / 19.5, 0))
english_font = pygame.font.Font('font/Brush Script.ttf', font_size)
chinese_font = pygame.font.Font('font/Chinese regular script.ttf', font_size)
chinese_title_font = pygame.font.Font('font/Chinese regular script.ttf', font_size*2)
chinese_help_font = pygame.font.Font('font/Regular Script.ttf', int(round(font_size/8*5, 0)))
num_font = pygame.font.Font('font/Chinese regular script.ttf', size)
font = pygame.font.Font('font/Chinese regular script.ttf', int(round(pix/26, 0)))
...
with open('topic.txt', mode='r', encoding='utf-8') as f:
    wiki_topic_lis = f.readlines()
    index = 0
    for i in wiki_topic_lis:
        wiki_topic_lis[index] = i.split('\n')[0]
        index += 1

with open('answer.txt', mode='r', encoding='utf-8') as f:
    wiki_answer_lis = f.readlines()
    index = 0
    for i in wiki_answer_lis:
        wiki_answer_lis[index] = i.split('\n')[0]
        index += 1
